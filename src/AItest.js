...
export default function AItest({
  ...
  // 🔑 從 .env 讀取固定的 Gemini API Key
  const GEMINI_KEY = process.env.REACT_APP_GEMINI_API_KEY || '';

  const ai = useMemo(() => {
    try {
      return GEMINI_KEY ? new GoogleGenAI({ apiKey: GEMINI_KEY }) : null;
    } catch {
      return null;
    }
  }, [GEMINI_KEY]);
  ...
  async function sendMessage(message) {
    ...
    if (!ai) {
      setError('系統尚未設定有效的 Gemini API Key，請在 .env 中設定 REACT_APP_GEMINI_API_KEY。');
      return;
    }
    setError('');
    setLoading(true);
